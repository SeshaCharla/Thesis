...
\begin{align}
    V_{scr} \dot{\con{NO_x}}^{out} &= F_{vol} \lr{\con{NO_x}^{in}-\con{NO_x}^{out}} - V_{scr} \lr{k_{scr} \con{NO_x}^{out} \con{NH_3}^{ads}} \label{eqn::nox_mass_bal}\\
    % ====
    V_{scr} \dot{\con{NH_3}}^{out} &= F_{vol} \lr{\con{NH_3}^{in}-\con{NH_3}^{out}} \notag \\
    & \quad  + V_{scr}\lr{ - k_{ads} \con{NH_3}^{out} \lr{\Gamma - \con{NH_3}^{ads}} + k_{des} \con{NH_3}^{ads}} \label{eqn::nh3_mass_bal}
    \\
    % ====
    \dot{\con{NH_3}}^{ads} &= k_{ads} \con{NH_3}^{out} \lr{\Gamma - \con{NH_3}^{ads}}
                            - k_{des} \con{NH_3}^{ads}
...
Furthermore, the continuous dynamics modeled in the above equations capture the high-frequency reaction kinetics coupled
