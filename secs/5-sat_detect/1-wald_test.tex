...
\end{align}
where $\hat \theta_{sat}$ is the parameter estimate obtained from the available measurements and $\theta^{dg}_{sat}$ is the parameter value corresponding to a degreened catalyst. The test statistic for this hypothesis testing problem can be constructed based on the parameter estimates and their variances. For a given data set $\pmb x$, let $p(\pmb x; \theta)$ denote the likelihood function of the data given the parameter $\theta$. Thus, we have the generalized likelihood ratio test (GLRT) \cite{kay1998detection} as follows:
\begin{align}
...
\begin{align}
        \lrb{I(\hat \theta_{sat})}_{ij} &=  \lrb{I(\theta_{sat})}_{ij} + \left. \frac{\partial \lrb{I(\theta_{sat})}_{ij}}{\partial \theta_{sat}} \right|_{\theta_{sat} = \hat \theta_{sat}} (\hat \theta_{sat} - \theta_{sat})
        \label{eqn::I_taylor}
\end{align}
...
\begin{align}
        ...
        \end{cases}
\end{align}
Using similar arguments as for the asymptotic equivalence of the Fisher
information matrix (\ref{eqn::I_asymp}) we get the equation
(\ref{eqn::I_asymptotic_equivalence}) which essentialy states that variances of
the parameter vectors is similar between aged and degreened catalysts and only
the mean is shifted.
\begin{align}
        I(\hat \theta_{sat}) (\hat \theta_{sat} - \theta^{dg}_{sat}) =
        I(\theta^{dg}_{sat}) (\hat \theta_{sat} - \theta^{dg}_{sat}) =
        I(\theta^{ag}_{sat}) (\hat \theta_{sat} - \theta^{dg}_{sat})
        \label{eqn::I_asymptotic_equivalence}
\end{align}
%%
% Write the physical interpretation of the above equation. Mean Shifted but the variance remains same for distribution.
%%
