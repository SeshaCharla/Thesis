...
\end{align}
%===
This quantity, $\alpha_{sat}$ can be estimated and changes with aging of the
catalyst based on the hypothesis that the aging changes the adsorption site
concentration. The change in $\alpha_{sat}$ with aging is demonstrated using
the test-cell experiments (Figures~\ref{fig::alpha_sat_ssd} and
\ref{fig::alpha_sat_iod}). The $NO_x$ sensor cross-sensitivity reduces the
difference in mean of $\alpha_{sat}$ between the degreened and aged catalysts,
but the aging effect is still discernible.
\begin{figure}[!ht]
