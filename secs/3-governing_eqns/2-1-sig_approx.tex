...
\subsection{Introducing catalyst saturation \label{sec::sat_cat}}
 The model developed so far assumes the catalyst remains unsaturated at all times. It does not account for cases where
 the catalyst becomes saturated or depleted during the sampling period. These effects can be incorporated using the
 switching function $g_{sat}$, defined as:
 \begin{align}
    g_{sat} (\sigma) = \begin{cases}
        ...
        0 & \text{if } \sigma < 0
    \end{cases}
    \label{eqn::g_sat_func}
\end{align}
Introducing the catalyst saturation effect using the switching function $g_{sat}$, we have the following two equations
describing the ammonia adsorption/desorption process dynamics:
\begin{align}
    \sigma^{ub}(k+1) &= \sigma(k) + t_s k_{ads} \con{NH_3}^{in} (k) \lr{\Gamma - \sigma(k)} \notag\\
        & \qquad - t_s \lr{k_{oxi} + k_{des}} \sigma(k) - t_s k_{scr} \con{NO_x}^{in}(k) \sigma(k) \\
    \sigma(k) &= g_{sat} \lr{\sigma^{ub}(k)}
\end{align}
