...
An alternative and concise derivation of the above model provides greater insight into the parametric switching conditions. We have the following equations describing the $NO_x$ reduction and ammonia adsorption in SCR-ASC system:
\begin{align}
...
\end{align*}
Multimplying both sides of the $\sigma(k+1)$ equation by $Q(k+1)$, we get
\begin{align*}
...
\begin{align*}
        ...
        & - t_s k_{scr}(k-1) u_1(k-1) \bar \eta_F (k)
\end{align*}
Parametrizing the above equation, with a chebushev basis polynomial temperature model for the rate constants (linear) and product of rate constants and $\Gamma$ (quadratic), we have the following:
\begin{align}
        \bar \eta_F (k+1) = \bar \eta_F (k) &+ u_{2F}(k-1)\phi_2(k-1) \theta_\Gamma \label{eqn::t2}\\
        & - u_{2F}(k-1) \bar \eta_F (k) \phi_1(k-1) \theta_{ads} \label{eqn::t3}\\
        & - \bar \eta_F (k) \phi_1(k-1) \theta_{od} \label{eqn::t4}\\
        & - u_1(k-1) \bar \eta_F (k) \phi_1(k-1) \theta_{scr} \label{eqn::t5}\\
        \text{where,} \qquad  & \notag\\
...
\end{align}
Terms, (\ref{eqn::t2}) and (\ref{eqn::t3}) of the right hand side form the adsorption dynamics, term (\ref{eqn::t4})
...
\end{align*}
\begin{align*}
        u_{2F}(k-1)\phi_2(k-1) \theta_\Gamma - u_{2F}(k-1) \bar \eta_F (k) \phi_1(k-1) \theta_{ads} \geq 0
\end{align*}
\begin{align*}
        \implies  \bm{- \phi_2^T(k-1) &  \bar \eta_F(k) \phi_1^T(k-1)} \bm{\theta_\Gamma \\ \theta_{ads}} \leq 0
\end{align*}
\begin{align}
        \implies  \underbrace{\bm{- \phi_2^T(k-1) &  \bar \eta_F(k) \phi_1^T(k-1)
                        & \pmb 0 & \pmb 0}}_{\phi_g(k)^T} \underbrace{\bm{\theta_\Gamma \\ \theta_{ads} \\ \theta_{od} \\ \theta_{scr}}}_{\theta_{NO_x}} \leq 0
...
adsorption dynamics switch to zero, and the dynamics switch to saturated catalyst mode. Thus, we end up with a switching condition that is parametrized linearly in terms of the model parameters reuslting in a \itbf{self-excited threshold nonlinear autoregressive with exogenous input} \cite{hansen1991institutional} model structure.
% =========================================

Also, $NO_x$ reduction reduction in a given time-step is always positive, i.e.,
\begin{align*}
        \bar \eta_F (k) \geq 0 \qquad \forall \quad k
\end{align*}
This condition is implicitly satisfied.
% =========================================
Incorporating the switching condition and the saturated and unsaturated $NO_x$ reduction dyanamics, we have the state-diagriam for the $NO_x$ reduction dyanimics as shown in Figure \ref{fig::no_x_swtiching}.
