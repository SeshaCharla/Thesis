...
\begin{align*}
        \lrf{\tau(k) k_{s2v} k_{scr}(k) u_1(k)} \Gamma  &= \frac{\tau_0}{F(k)} \times k_{s2v} \times \phi(k)^T \theta_{scr} \times u_1(k) \times \Gamma\\
        % ==
        &= \lrf{\lrb{\frac{u_1(k)}{F(k)}} \phi^T(k) } \lrf{\Gamma \tau_0 k_{s2v} \theta_{scr}}
\end{align*}
\begin{align}
        Let, \qquad  \Gamma \tau_0 k_{s2v} \theta_{scr} &= \theta_{\Gamma scr}\\
        \lrf{\tau(k) k_{s2v} k_{scr}(k) u_1(k)} \Gamma  &=
        \lrb{\frac{u_1(k)}{F(k)}} \phi^T(k)  \theta_{\Gamma scr}
        \label{eqn::gamma_scr}
\end{align}
%===
...
\begin{align}
        ...
        +\lrb{\frac{u_1(k)}{F(k)}} \phi_{NO_x}^T \theta_{NO_x}\\
        %===
        f_{\Gamma}(k) &= \lrb{\frac{u_1(k)}{F(k)}} \phi^T(k)  \theta_{\Gamma scr}
\end{align}
