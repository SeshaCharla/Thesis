...
The linear programming problem is solved using a quadratic polynomial
approximation for $\pmb \phi \pmb \theta_{sat}$. Both $k_{scr/ads}$ and
$\Gamma$ are assumed to be monotonic functions of temperature but in opposing
directions. Thus, a linear model for the product will not capture the change in
monotonicity of the product that happens at a particular temperature. After a
systematic analysis of various polynomial orders and temperature partitions, a
quadratic model with two partitions was found to be the best approximation with
minimum prediction error. The parameter estimates of the saturated model are
tabulated in Table~\ref{tab::sat_parm_est}.
...
from the data are shown in Figures~\ref{fig::eta_bounds_cftp}, \ref{fig::eta_bounds_hftp}, and \ref{fig::eta_bounds_rmc} for different drive cycles.
