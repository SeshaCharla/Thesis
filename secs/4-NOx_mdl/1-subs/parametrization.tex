...
Further, for numerical stability \cite{press2003numerical}, the given temperature range in mapped to $[-1,1]$ and chebyshev polynomial basis \cite{trefethen2019approximation} are used instead of standard polynomials. We have the first and second order chebyshev basis as follows:
\begin{align}
...
\begin{minipage}{0.49\textwidth}
        \begin{align}
                \phi_{\eta}(k) &= \eta(k) \lrb{\frac{u_1(k)}{F(k)}}
                                \bm{\lrb{\frac{u_2(k-1)}{u_1(k-1)}}  \phi^T(k-1) \\
                                         \lrb{\frac{F(k-1)}{u_1(k-1)}} \phi^T(k-1)     \\
                                                 F(k-1) \phi^T(k)
                                                }
        \end{align}
\end{minipage}
...
\begin{minipage}{0.49\textwidth}
        \begin{align}
                \phi_{\Gamma} (k) &= \lrb{\frac{u_1(k)}{F(k)} } \lrb{ \frac{u_2(k-1)}{F(k-1)}} \phi(k-1)
        \end{align}
\end{minipage}
\begin{minipage}{0.49\textwidth}
        \begin{align}
...
$ \phi_\eta$ can be further simplified to avoid the multiplication and division of the same signals that results in noise amplification as follows:

\begin{align}
     \phi_{\eta}(k)
                        &= \lrb{\frac{u_1(k)}{F(k)}}
                                \bm{\lrb{1 - \frac{x_1(k)}{u_1(k-1)}} u_2(k-1) \phi^T(k-1) \\
                                     \lrb{1 - \frac{x_1(k)}{u_1(k-1)}}   F(k-1) \phi^T(k-1)     \\
                                        \eta(k) F(k-1) \phi^T(k)
                                                }
\end{align}
