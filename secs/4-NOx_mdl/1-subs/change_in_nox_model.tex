...
Let \textbf{$\eta$ denote the change in concentration from inlet to outlet $NO_x$ in one sample}. This is the change in
concentration in the exhaust due to the $NO_x$ reduction.
%
\begin{align}
...
\begin{multline}
        \sigma(k+1) = \sigma(k) - \sigma(k) t_s k_{ads}(k) \con{NH_3}^{in}(k)
...
\begin{multline*}
         \sigma(k) = \sigma(k-1)\lrf{1 -  t_s k_{ads}(k-1) \con{NH_3}^{in}(k-1)
...
Let
\begin{align}
        \gamma_{proc}(k-1) &= \lrf{1 -  t_s k_{ads}(k-1) \con{NH_3}^{in}(k-1)
                        -  t_s k_{scr}(k-1) u_1(k-1)
                        -  t_s k_{od}(k-1)}
...
\end{multline}
Explicitly writing the individual terms leads to:
\begin{align*}
...
\begin{itemize}
        \item[$A1.$] The temperature does not change significantly across contiguous samples, i.e., $T(k-1) \approx T(k)$.
        \begin{align}
